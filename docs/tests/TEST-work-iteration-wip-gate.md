# TEST: Work Iteration WIP Gate
> Task: T-282
> Date: 2026-06-29

## Cases

1. Hub shows Iteration card
   - Open Work Hub.
   - Expect a visible card titled `迭代` after `任务表`.
...
3. Owner and admin can enter placeholder
   ...
   - Click `迭代`.
   - Expect Work Hub to hide and the `迭代` placeholder view to show.
   - Click back.
...
5. Backend security boundary remains explicit
   - Confirm no new `/api/iteration/*` endpoint was added.
   - Confirm front-end code documents that future `/api/iteration/*` endpoints must use `AdminUserId`.
