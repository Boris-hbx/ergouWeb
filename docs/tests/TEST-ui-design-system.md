...
> 覆盖任务: T-238, T-239, T-240, T-241, T-242
> 范围: Next Web 一期按钮层、基础视觉 token、弹窗/抽屉/浮层、工具栏/视图切换、表格/列表/卡片密度与首批迁移页面
...
- `components.css` 提供表格 surface、密度表格、重复卡片和列表分隔线基础类。
- 工作任务表使用薄边框、紧凑行高、稳定 hover/selected 态，不依赖大阴影或大圆角表达层级。
- 工作看板卡、人员卡、分布卡只作为重复项卡片，半径和 padding 收敛，不出现卡片套卡片式漂浮效果。
- 干系人表/卡沿用工作表格和重复卡片密度，关系侧栏使用边框而非阴影表达容器边界。
- Admin 表格、token 表、摘要卡、系统卡和风险卡保持扫描密度，行内操作不被大留白挤压。
- Insight 列表表格、录入区、候选/来源卡和详情卡使用边框主导的管理型 surface。
